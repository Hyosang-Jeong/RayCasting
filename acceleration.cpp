...
std::uniform_real_distribution<> myrandom3(0.0, 0.5);

/////////////////////////////
// Vector and ray conversions
Ray RayFromBvh(const bvh::Ray<float> &r)
{
    return Ray(vec3FromBvh(r.origin), vec3FromBvh(r.direction), myrandom3(RNGen3));
}
...
std::optional<Intersection> BvhShape::intersect(const bvh::Ray<float>& bvhray) const
{
    ...
    if (shape->intersect(ray, intersection) == false)
        return std::nullopt;
