	...
	const std::vector<float>& f)
{
	...
	else if (c == "ambient")
	{
		...
		if (f.size() == 13)
			currentMat->setTexture(strings[12].c_str());
	}
		...
		// syntax: sphere x y z   r
		// Creates a Shape instance for a sphere defined by a center and radius
		mbSphere* sphere = new mbSphere(sphMesh, vec3(f[1], f[2], f[3]), vec3(f[4], f[5], f[6]), f[7], currentMat);
		shapes.push_back(sphere);
		if (sphere->material->isLight())
			lights.push_back(sphere);
...
#pragma omp parallel for schedule(dynamic, 1) // Magic: Multi-thread y loop
		...
		for (int y = 0; y < height; y++)
		{
			for (int x = 0; x < width; x++)
			{
				...
				Ray ray{ eye, normalize(dx * X + dy * Y - Z), (float)myrandom(RNGen) };
	...
	{
		{
			...
			Ray I = Ray{ P.P, wi,0.f};
			auto I_result = scene->acceleration->intersect(I);
			if (p > 0 && I_result.object != nullptr && I_result.P == L.P)
			{
				vec3 f = P.object->EvalScattering(P.t, wo, normalize(N), wi, scene->mode,P.u,P.v);
...

		{
			...
			vec3  f = P.object->EvalScattering(P.t,wo, N, wi, scene->mode, P.u, P.v);
