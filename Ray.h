...
public:
	...
	virtual bool isBlured() { return false; }
	vec3 EvalScattering(float t, vec3 wo, vec3 N, vec3 wi, Mode mode,float u, float v);
	Material* material;
	MeshData* meshdata;
	mat4 modelTR;
	vec3 center;
...
class mbSphere : public Shape //motion blur sphere
{
public:
	mbSphere(MeshData* meshData, const vec3 A, const vec3 B, const float r, Material* mat);
	virtual bool isBlured() override
	{ 
		return true; 
	}
	vec3 Pt(float time);
...
public:
	...
	virtual  bool intersect(Ray, Intersection&) override;
	vec2 calculateUV(vec3 p, vec3 n);
	vec3 base;
	vec3 diag;
};
